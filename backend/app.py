...
if DATABASE_URL:
    ...
    logger.info(f"✓ Using MySQL at {MYSQL_HOST}:{MYSQL_PORT}/{MYSQL_DB}")
...
if not SECRET_KEY:
    ...
    if IS_PRODUCTION:
        logger.warning("⚠️  WARNING: SECRET_KEY not set in production environment!")
...
logger.info(f"✓ Connecting to MySQL: {MYSQL_USER}@{MYSQL_HOST}/{MYSQL_DB}")
...
@limiter.limit("10 per minute")  # Rate limiting: 10 attempts per minute
def login():
    try:
        data = request.json
        ...
        if user_type == 'admin':
            ...
            logger.warning(f"⚠️  Failed admin login attempt: {username}")
            return jsonify({'success': False, 'message': 'Invalid credentials'}), 401

        elif user_type == 'teacher':
            teacher = Teacher.query.filter_by(username=username).first()
            if teacher and verify_password(password, teacher.password):
                logger.info(f"✓ Teacher login successful: {username}")
                return jsonify({
                    'success': True,
                    'userType': 'teacher',
                    'teacher': teacher.to_dict()
                }), 200
            logger.warning(f"⚠️  Failed teacher login attempt: {username}")
            return jsonify({'success': False, 'message': 'Invalid credentials'}), 401
...
def init_database_auto():
    """Automatically initialize database on startup if needed"""
    try:
        with app.app_context():
            ...
            if not existing_admin:
                ...
                logger.info(f"✓ Admin account exists (username: {existing_admin.username})")

            # Check if classrooms already exist
            if Classroom.query.count() == 0:
                logger.info("📚 Creating school classrooms...")
                ...
                db.session.commit()
                logger.info("✓ Created 28 classrooms (16 Junior High + 12 Senior High)")
            else:
                logger.info(f"✓ Classrooms exist ({Classroom.query.count()} total)")
...
if __name__ == '__main__':
    ...
    debug = os.getenv('FLASK_DEBUG', 'False').lower() == 'true' if IS_PRODUCTION else True
